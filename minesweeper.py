...
def generate_numbers(x, y, coords):
    ...
    for mine_around in (around(it) for it in coords):
        for coord in in_bounds(x, y, mine_around):
...
def draw(x, y, mine_count):
    mines = generate_mines(x, y, mine_count)

    return "\n".join(
        [
            "".join([f"||{it}||" for it in row])
            for row in generate_board(
            x, y, mines, generate_numbers(x, y, mines)
            )
        ]
    )
