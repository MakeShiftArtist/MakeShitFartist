...
class Trello:
    ...
    class Info:
        def client(api_key, api_secret, token):
            return TrelloClient(
                api_key=Common_info.trello.api_key,
                api_secret=Common_info.trello.api_secret,
                token=Common_info.trello.token
            )
...
class Embeds:
    ...
    @staticmethod
    def loading():
        title = '<a:loading:774385294106689557> Loading...'
    ...
    @staticmethod
    def error_embed(ctx, error, commands, command=None) -> discord.Embed:
    ...
    @staticmethod
    def unknown_error_embed(ctx, error, command=None) -> discord.Embed:
        embed = discord.Embed(
    ...
    def error_handler(ctx, error, commands, command=None):
        embed = Embeds.error_embed(ctx, error, commands, command)
        log_embed = None
        if embed is None:
            embed = Embeds.unknown_error_embed(ctx, error, command)
...
class Database:
    ...
    def get_select(self, select_statement):
